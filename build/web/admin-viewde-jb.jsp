...
<%@ page import="Dbconn.dbconn"%>
          ...
          <div class="container-xxl flex-grow-1 container-p-y">
            <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">View/</span> Job Details</h4>
...
try {
  ...
  PreparedStatement countStatement = connection.prepareStatement("SELECT COUNT(*) FROM jobs j INNER JOIN company c ON j.companyid = c.companyid");
   ResultSet countResult = countStatement.executeQuery();
    ...
    // Get current page from parameter or set it to 1
    int currentPage = (request.getParameter("page") != null) ? Integer.parseInt(request.getParameter("page")) : 1;
    // Ensure currentPage is within valid range
    currentPage = Math.max(1, Math.min(currentPage, totalPages));

    // Calculate start record index for current page
    int startRecord = (currentPage - 1) * recordsPerPage;

    // Fetch job postings and related company information using JOIN with LIMIT
    PreparedStatement statement = connection.prepareStatement(
        "SELECT j.*, c.company, c.address, c.cimage, c.email FROM jobs j INNER JOIN company c ON j.companyid = c.companyid LIMIT ?, ?");
...
//String companyImageBase64 = new String(Base64.getEncoder().encode(companyImageBytes));
        ...
        %>  
            ...
            <td><%= jobNature %></td>
              ...
              <td><textarea rows="1" cols="18" ><%= Skills %></textarea></td>
                  <td><textarea rows="1" cols="18" ><%= experienceEducation %></textarea></td>
            
         
             
<td style="color:
  <% String action = resultSet.getString("status");
...
<div class="row">
    <div class="btn-group">
        <form action="update-status.jsp" method="post">
            <input type="hidden" name="jobid" value="<%= resultSet.getString("jobid") %>">
            <input type="hidden" name="newstatus" value="Accepted">
            <input type="hidden" name="source" value="jobid"> <!-- Specify the source here -->
            <button type="submit" class="btn-xs btn-success">Accept (Job)</button>
        </form>
        &nbsp;&nbsp;
        <form action="update-status.jsp" method="post">
            <input type="hidden" name="jobid" value="<%= resultSet.getString("jobid") %>">
            ...
            <input type="hidden" name="source" value="jobid"> <!-- Specify the source here -->
            <button type="submit" class="btn-xs btn-danger">Decline (Job)</button>
        </form>
    </div>
</div>
