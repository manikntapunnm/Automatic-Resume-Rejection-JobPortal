      ...
      <div class="layout-page">
        ...
        <div class="content-wrapper">
          ...
          <div class="container-xxl flex-grow-1 container-p-y">
            <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Hired /</span> Candidates</h4>
...
try {
 ...
 int totalRecords = 0;
    ...
    countStatement.setString(1, apply);
...
//    "SELECT a.*, j.salary,j.jobnature,j.experience_education, c.company,c.address,ca.name,ca.image " +
//    "FROM applications a " +
//    "INNER JOIN jobs j ON a.jobid = j.jobid " +
//    "INNER JOIN company c ON a.companyid = c.companyid " +
//    "INNER JOIN candidates ca ON a.candidateid = ca.candidateid " +
//    "WHERE a.application = ?" + "LIMIT ?,?");
// statement.setString(1, apply);
//statement.setInt(2, startRecord);
//statement.setInt(3, recordsPerPage);
PreparedStatement statement = connection.prepareStatement(
    "SELECT a.*, j.salary, j.jobnature, j.experience_education, c.company, c.address,c.email, ca.name, ca.image " +
    "FROM applications a " +
    "INNER JOIN jobs j ON a.jobid = j.jobid " +
    "INNER JOIN company c ON a.companyid = c.companyid " +
    "INNER JOIN candidates ca ON a.candidateid = ca.candidateid " +
    "WHERE a.application = ? AND c.email = ? " +
    "LIMIT ?, ?");
statement.setString(1, apply);
statement.setString(2, email); // Set the email value
statement.setInt(3, startRecord);
statement.setInt(4, recordsPerPage);


    ResultSet resultSet = statement.executeQuery();
     int sno = startRecord + 1; // Counter for serial number
  %>

            <!-- Responsive Table -->
            <div class="card">
              <h5 class="card-header">Rejection Details</h5>
    ...
    while (resultSet.next()) {
      ...
      //  out.println(jobNature);
       ...
       //   out.println(applicationStatus);
        String exp = resultSet.getString("experience_education");
